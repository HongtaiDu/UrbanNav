...
{
    int err;
    uint16_t interval_units = ms_to_adv_units(interval_ms);
    ...
    printk("Advertising started at %u ms (units=0x%04x)\n",
           interval_ms, interval_units);

    err = set_adv_tx_power(adv_tx_power_dbm);
    if (err) {
        printk("Warning: advertising started, but TX power update failed\n");
    } else {
        printk("Advertising TX power active: %d dBm\n", adv_tx_power_dbm);
    }
